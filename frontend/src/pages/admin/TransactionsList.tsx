...
export const TransactionsList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    fetchTransactions();
  }, [type, branchId, startDate, endDate, page]);
  ...
  const inputCls =
    'w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent placeholder-slate-400 dark:placeholder-slate-500';
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 md:ml-64 pt-16 md:pt-0">
      ...
      <div className="border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900/50 px-6 py-6">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-xl font-semibold text-slate-900 dark:text-white">Transacciones</h1>
        </div>
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-5">
            <p className="text-xs font-medium text-slate-400 uppercase tracking-wider mb-3">Neto (página)</p>
            <p className="text-2xl font-bold text-slate-900 dark:text-white">{fmt(totalAmount)}</p>
            <p className="text-xs text-slate-400 mt-1">{transactions.length} transacciones</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-5">
            ...
            <p className="text-2xl font-bold text-red-600 dark:text-red-400">-{fmt(purchasesAmount)}</p>
            <p className="text-xs text-slate-400 mt-1">{purchasesCount} transacciones</p>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-5">
            ...
            <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">+{fmt(rechargesAmount)}</p>
            <p className="text-xs text-slate-400 mt-1">{rechargesCount} transacciones</p>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-2">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-medium">Sin resultados</p>
              <p className="text-sm text-slate-400 dark:text-slate-600">Ajusta los filtros para ver transacciones</p>
            </div>
          ) : (
            <>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-100 dark:border-slate-800">
                      ...
                      <th className="text-right py-3 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider">Monto</th>
                      <th className="text-right py-3 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider hidden lg:table-cell">Antes</th>
                      <th className="text-right py-3 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider hidden lg:table-cell">Después</th>
                      <th className="text-left py-3 px-4 text-xs font-semibold text-slate-400 uppercase tracking-wider hidden xl:table-cell">Descripción</th>
                      ...
                      return (
                        <tr key={tx.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                          ...
                          <td className="py-3 px-4">
                            <p className="font-medium text-slate-800 dark:text-slate-200 truncate max-w-[140px]">{tx.user?.name || '—'}</p>
                          ...
                          <td className="py-3 px-4 text-right text-xs text-slate-500 dark:text-slate-400 tabular-nums hidden lg:table-cell">
                            {fmt(tx.balanceAfter)}
                          </td>
                          <td className="py-3 px-4 text-xs text-slate-400 max-w-xs truncate hidden xl:table-cell">
                            {tx.description || '—'}
                          </td>
              ...
              {pagination && pagination.pages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 dark:border-slate-800">
                  <span className="text-xs text-slate-400">
                    ...
                    <span className="ml-2 text-slate-300 dark:text-slate-600">({pagination.total.toLocaleString('es-MX')} total)</span>
                  </span>
                  <div className="flex items-center gap-1">
                    ...
                    <span className="px-3 py-1 text-xs font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-lg">
                      {pagination.page}
                    </span>
                    <button
